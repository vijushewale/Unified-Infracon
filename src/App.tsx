...
export default function App() {
  ...
  const startListening = async () => {
    ...
    if (!SpeechRecognition) {
      setRecognitionError("Speech recognition is not supported in this browser. Please type your phrase below. / आपल्या ब्राउझरमध्ये स्पीच रेकग्निशन सपोर्टेड नाही.");
      return;
    }

    // Proactively request native microphone permission first.
    // This is a robust solution for iframe/sandbox and mobile browser webviews
    // to trigger the native permission prompt before initializing Web Speech API.
    try {
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        // Stop all tracks immediately so the mic active icon doesn't linger
        stream.getTracks().forEach((track) => track.stop());
      }
    } catch (err: any) {
      console.warn("Microphone access via getUserMedia was rejected or failed:", err);
      setRecognitionError(
        "Microphone permission denied! Click 'Open App in New Tab' below or click the camera/mic icon in your browser URL bar to allow microphone access. \n\n(मायक्रोफोन परवानगी नाकारली! कृपया ब्राउझरच्या URL बारमधील कॅमेरा/मायक्रोफोन चिन्हावर क्लिक करून परवानगी द्या किंवा 'नवीन टॅबमध्ये ॲप उघडा' वर क्लिक करा.)"
      );
    ...
    try {
      ...
      rec.onerror = (event: any) => {
        ...
        if (event.error === "not-allowed") {
          setRecognitionError(
            "Microphone permission denied! Please reset your browser mic permissions or open the app in a new tab. \n\n(मायक्रोफोन परवानगी नाकारली! कृपया परवानग्या रिसेट करा किंवा ॲप नवीन टॅबमध्ये उघडा.)"
          );
        } else {
      ...
      setRecognitionError("Failed to initiate microphone / मायक्रोफोन सुरू करण्यास अपयशी.");
      setIsListening(false);
    }
  };
        ...
        {needsAuth ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <section className="lg:col-span-5 flex flex-col gap-6">
              ...
              {/* Google Sheets Cloud Sync & Mic Troubleshooter */}
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 rounded-3xl p-6 shadow-xl relative overflow-hidden flex flex-col gap-4">
                <h3 className="text-sm font-display font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider flex items-center gap-2">
                ...
                {isOfflineMode ? (
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 text-left">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        <h4 className="font-semibold text-amber-800 dark:text-amber-400 text-xs">
                          Offline Mode Active (Local Storage)
                        </h4>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 leading-normal">
                          Your voice entries are currently saving locally to your device. Connect your Google account below to automatically sync and write to your Google Sheets ledger.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 text-left">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        <h4 className="font-semibold text-emerald-800 dark:text-emerald-400 text-xs">
                          Cloud Sync Active (Connected)
                        </h4>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 leading-normal truncate">
                          Signed in as: <span className="font-mono text-slate-700 dark:text-slate-300 font-semibold">{user?.email || "Google User"}</span>
                        </p>
                        {sheetUrl && (
                          <p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-1.5 font-medium flex items-center gap-1.5">
                            <Check className="h-3.5 w-3.5" /> Connected to "Unified Infracon" spreadsheet
                          </p>
                        )}
                      </div>
                    </div>
                ...
                {/* Google Sync Action Buttons */}
                <div className="flex flex-col gap-2.5">
                  {isOfflineMode ? (
                    <button
                      onClick={handleGoogleSignIn}
                      disabled={isLoggingIn || isEnsuringSheet}
                      className="w-full py-3 bg-slate-900 dark:bg-slate-950 text-white hover:bg-slate-800 dark:hover:bg-slate-900 border border-slate-800 rounded-2xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-2"
                    >
                      {isLoggingIn || isEnsuringSheet ? (
                      ...
                      Connect Google Sheets / गुगल शीट जोडा
                    </button>
                  ) : (
                    <div className="flex gap-2">
                      {sheetUrl && (
                        <a
                          ...
                          className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-1.5"
                        >
                      ...
                      <button
                        ...
                        className="flex-1 py-3 bg-slate-100 hover:bg-rose-500/10 dark:bg-slate-800 dark:hover:bg-rose-500/10 text-slate-600 dark:text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 rounded-2xl text-xs font-bold transition-all border border-slate-200/50 dark:border-slate-800"
                      >
                        Disconnect Sync
                      </button>
                    </div>
                ...
                {/* Microphone / Tab Helper for Sandbox Previews */}
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 text-left">
                  <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-3.5">
                    <p className="text-[11px] font-semibold text-indigo-700 dark:text-indigo-400 flex items-center gap-1.5 mb-1">
                      <Mic className="h-4 w-4 animate-pulse shrink-0" />
                      Mic & Speech Not Working?
                    </p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-normal">
                      Browser sandboxes and iframes often block microphone access. Open the app in a new tab to enable full voice input, Marathi/Hindi speech dictation, and voice confirmations!
                    </p>
                    <button
                      onClick={() => window.open(window.location.href, "_blank")}
                      className="mt-2.5 w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] font-bold transition-all flex items-center justify-center gap-1"
                    >
