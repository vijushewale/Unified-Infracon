...
export default function App() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.error(err);
      showError("Sign in failed. Ensure popups are allowed.");
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:px-8">
        ...
        {needsAuth ? (
          <div className="max-w-xl mx-auto my-12 py-10 px-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl shadow-xl text-center glow-blue transition-all duration-300">
            ...
            {/* Official Google Sign-In Button styled perfectly */}
            <button 
              onClick={handleLogin}
              disabled={isLoggingIn}
              id="google-signin-btn"
              className="w-full max-w-xs mx-auto py-3.5 px-5 bg-white border border-slate-300 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700/80 text-slate-700 dark:text-slate-200 font-medium rounded-xl flex items-center justify-center gap-3 shadow-sm hover:shadow transition-all disabled:opacity-50"
            >
              {isLoggingIn ? (
